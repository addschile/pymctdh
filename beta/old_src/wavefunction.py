...
class Wavefunction(object):
    ...
    def __init__(self, nel, nmodes, nspfs, npbfs, A=None, spfs=None, 
        ...
        npbfs - list or np.ndarray, number of primitive basis functions for each
                mode
        ^TODO need better description for combined mode case
        psi - np.ndarray(dtype=complex), array containing the mctdh core tensor
              and single-particle functions
        npsi - int, size of psi array
        psistart - np.ndarray
        psiend - np.ndarray
        A - np.ndarray, the core mctdh tensor
        spfs - np.ndarray, array containing the spfs as a vecotr of pbfs
               stored continuously as a 1D array
        spfstart - where the spfs for a particular mode start in self.spfs
        spfstart - where the spfs for a particular mode start in self.spfs
        combined
        cmodes
        npsi
