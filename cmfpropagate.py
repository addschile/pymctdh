...
def cmfadapropagate(times, ham, pbfs, wf, filename, tol=1.e-4):
    """Propagate MCTDH wavefunction based on Dirac-Frenkel variational
    principle. Uses the constant mean field scheme in which 
    ...
    for i in range(len(times)-1):
        ...
        while abs(t0 - times[i+1]) > 1.e-12:
            ...
            mfs,rhos,projs = cmfprecomputemf(ham,opspfs,opips,spfovs,wf)
            # integrate coefficients one half timestep forward
            Aintegrate(t0, t0+0.5*dt, 0.5*dt, wf, ham, opips, spfovs)
            # integrate spfs one half timestep forward
            spfstmp = wf.copy('spfs')
            spfsintegrate(t0, t0+0.5*dt, 0.5*dt, wf, ham, pbfs, 
                          cmfflag=True, eq='spfs', opspfs=opspfs, opips=opips, 
                          spfovs=spfovs, mfs=mfs, rhos=rhos, projs=projs)
            # precompute new stuff for propagation
            opspfs,opips,spfovs = cmfprecomputemels(ham,pbfs,wf)
            ...
            # integrate spfs one half timestep forward
            wf.spfs = deepcopy(spfstmp)
            spfsintegrate(t0, t0+dt, dt, wf, ham, pbfs, 
                          cmfflag=True, eq='spfs', opspfs=opspfs, opips=opips, 
                          spfovs=spfovs, mfs=mfs, rhos=rhos, projs=projs)
            #precompute new stuff for propagation
            opspfs,opips,spfovs = cmfprecomputemels(ham,pbfs,wf)
            # integrate coefficients one half timestep backward for error checking
            Atmp = wf.copy('A')
            Aintegrate(t0+0.5*dt, t0, -0.5*dt, wf, ham, opips, spfovs)
            # compute error in cmf step
            diff = 0.0
            for j in range(nel):
                diff += np.linalg.norm(A0[j]-wf.A[j])
            tot = 0.0
            for j in range(nel):
                tot += np.sum(np.abs(A0[j]))
            err = diff/tot
            #print(count,dt,err,tol)
            if err > tol:
                # if error is too large adapt timestep
                dt_guess = dt*tol*tot/diff
                #print(dt_guess,np.log(tol/err),np.log(tol / err)*(dt_guess-dt))
                dt = dt - np.log(tol / err)*(dt_guess-dt)
                if dt < dt_guess:
                    dt = dt_guess
                #dt /= 2.0
                wf.spfs = deepcopy(spfs0)
                wf.A = deepcopy(A0)
                count += 1
            else:
                # if error is fine do final A update and move on
                wf.A = deepcopy(Atmp)
                Aintegrate(t0+0.5*dt, t0+dt, 0.5*dt, wf, ham, opips, spfovs)
                t0 += dt
        if i%1==0:
            norm = wf.norm()
            f.write('%.8f\n'%(norm))
