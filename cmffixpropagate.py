...
from eom import eom_coeffs,eom_spfs
...
def cmffixpropagate(times, ham, pbfs, wf, filename):
    ...
    ode_coeffs_args = [wf.psiend[0,-1],wf.nel,wf.nmodes,wf.nspfs,wf.npbfs,
                       wf.psistart,wf.psiend,uopips,copips,huelterms,hcelterms,
                       spfovs]
    rcoeffs.set_f_params(*ode_args)
    indf = wf.psistart[0,-1]
    ...
    rspfs.set_integrator('zvode',method='adams',order=12,atol=1.e-8,rtol=1.e-6,
                         nsteps=1000,first_step=0,min_step=0,max_step=dt)
    ode_spfs_args = [wf.psiend[0,-1],wf.nel,wf.nmodes,wf.nspfs,wf.npbfs,
                     wf.psistart,wf.psiend,uopips,copips,huelterms,hcelterms,
                     spfovs]
