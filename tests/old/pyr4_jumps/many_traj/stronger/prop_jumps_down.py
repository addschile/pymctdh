...
if __name__ == "__main__":
    ...
    for traj in range(ntraj):
        wfout = vmfpropagatejumps(times, ham, pbfs, Ls, LdLs, wf.copy(), 'pyr4_jumps_down_traj_%d.txt'%(traj))
        #wfout = vmfpropagatejumps(times, ham, pbfs, Ls, LdLs, wf.copy(), 'pyr4_jumps_down_traj_%d.txt'%(traj), seed=2)
        #for alpha in range(nel):
        #    print('el state %d'%(alpha))
        #    for mode in range(nmodes):
        #        print('mode %d'%(mode))
        #        ind0 = wfout.spfstart[alpha,mode]
        #        nspf = wfout.nspfs[alpha,mode]
        #        npbf = wfout.npbfs[mode]
        #        for i in range(nspf):
        #            print('spf %d'%(i))
        #            print(wfout.spfs[alpha][ind0:ind0+npbf])
        #            ind0 += npbf
        #        print('')
        #    print('')
        #    print('')
