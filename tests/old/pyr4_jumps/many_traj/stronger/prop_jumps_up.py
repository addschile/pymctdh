...
if __name__ == "__main__":
    ...
    hterms.append({'coeff': -gam*0.5j, 'units': 'au', 'modes': 0, 'elop':   '1', 'ops': 'n+1'}) # parameters for effective hamiltonian
    hterms.append({'coeff': -gam*0.5j, 'units': 'au', 'modes': 1, 'elop':   '1', 'ops': 'n+1'}) # parameters for effective hamiltonian
    hterms.append({'coeff': -gam*0.5j, 'units': 'au', 'modes': 2, 'elop':   '1', 'ops': 'n+1'}) # parameters for effective hamiltonian
    hterms.append({'coeff': -gam*0.5j, 'units': 'au', 'modes': 3, 'elop':   '1', 'ops': 'n+1'}) # parameters for effective hamiltonian
    ham = Hamiltonian(nmodes, hterms, pbfs=pbfs)

    # make Lindblad operators
    Ls = []
    term = {'coeff': np.sqrt(gam), 'units': 'au', 'modes': 0, 'elop': '1', 'ops': 'adag'}
    Ls.append( QOperator(nmodes, term, pbfs=pbfs) )
    term = {'coeff': np.sqrt(gam), 'units': 'au', 'modes': 1, 'elop': '1', 'ops': 'adag'}
    Ls.append( QOperator(nmodes, term, pbfs=pbfs) )
    term = {'coeff': np.sqrt(gam), 'units': 'au', 'modes': 2, 'elop': '1', 'ops': 'adag'}
    Ls.append( QOperator(nmodes, term, pbfs=pbfs) )
    term = {'coeff': np.sqrt(gam), 'units': 'au', 'modes': 3, 'elop': '1', 'ops': 'adag'}
    Ls.append( QOperator(nmodes, term, pbfs=pbfs) )
    # make Lindblad waiting time operators
    #LdLs = []
    #term = {'coeff': gam, 'units': 'au', 'modes': 0, 'elop': '1', 'ops': 'adag*a'}
    #LdLs.append( QOperator(nmodes, term, pbfs=pbfs) )
    #term = {'coeff': gam, 'units': 'au', 'modes': 1, 'elop': '1', 'ops': 'adag*a'}
    #LdLs.append( QOperator(nmodes, term, pbfs=pbfs) )
    #term = {'coeff': gam, 'units': 'au', 'modes': 2, 'elop': '1', 'ops': 'adag*a'}
    #LdLs.append( QOperator(nmodes, term, pbfs=pbfs) )
    #term = {'coeff': gam, 'units': 'au', 'modes': 3, 'elop': '1', 'ops': 'adag*a'}
    #LdLs.append( QOperator(nmodes, term, pbfs=pbfs) )
    LdLs = []
    ...
    LdLs.append( QOperator(nmodes, term, pbfs=pbfs) )
    term = {'coeff': gam, 'units': 'au', 'modes': 2, 'elop': '1', 'ops': 'n+1'}
    LdLs.append( QOperator(nmodes, term, pbfs=pbfs) )
    term = {'coeff': gam, 'units': 'au', 'modes': 3, 'elop': '1', 'ops': 'n+1'}
    LdLs.append( QOperator(nmodes, term, pbfs=pbfs) )
    ...
    for traj in range(ntraj):
        #wfout = vmfpropagatejumps(times, ham, pbfs, Ls, LdLs, wf.copy(), 'pyr4_jumps_up_traj_%d.txt'%(traj))
        wfout = vmfpropagatejumps(times, ham, pbfs, Ls, LdLs, wf.copy(), 'pyr4_jumps_up_traj_%d.txt'%(traj), seed=2)
