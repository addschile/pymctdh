...
def construct_sys():
    ...
    Q10a = np.sqrt(gam)*tensor(eye(2), a10a.dag(),  eye(n6a),  eye(n1),  eye(n9a))
    Q6a  = np.sqrt(gam)*tensor(eye(2),  eye(n10a), a6a.dag(),  eye(n1),  eye(n9a))
    Q1   = np.sqrt(gam)*tensor(eye(2),  eye(n10a),  eye(n6a), a1.dag(),  eye(n9a))
    Q9a  = np.sqrt(gam)*tensor(eye(2),  eye(n10a),  eye(n6a),  eye(n1), a9a.dag())
    ...
    return H,psi,p1,p2,Q10a,Q6a,Q1,Q9a
...
def main():
    ...
    #times = np.arange(0.0,60.0,dt)*fs2au

    # construct system
    H,psi0,p1,p2,Q10a,Q6a,Q1,Q9a = construct_sys()

    # run dynamics
    #results = mcsolve(H, psi0, times, c_ops=[Q10a,Q6a,Q1,Q9a], e_ops=[p1,p2], ntraj=100, options=Options(num_cpus=1), progress_bar=True)
    #results = mcsolve(H, psi0, times, c_ops=[Q10a,Q6a,Q1,Q9a], e_ops=[p1,p2], ntraj=10, options=Options(num_cpus=1), progress_bar=True)
    results = mcsolve(H, psi0, times, c_ops=[Q10a,Q6a,Q1,Q9a], e_ops=[p1,p2], ntraj=1, options=Options(seeds=2,num_cpus=1), progress_bar=True)
    output = np.zeros((len(times),3))
    output[:,0] = times/fs2au
    output[:,1] = results.expect[0]
    output[:,2] = results.expect[1]
    np.savetxt('pyr4_qutip_up.txt', output)
